...
class CloudWatchLogsAgentSetup:
    ...
    def install_awslogs_cli(self):
        ...
        if self.is_standalone:
            ...
            """ Note: As part of the standalone installation feature, the version of the awscli that the Logs plugin dependes on has been bumped up
            and so is the version number of the Logs Plugin itself.For standalone installation, the dependency packages will be vended in an S3 bucket,
			so using the bumped up version here. For standard installation, the pypy repository would need awscli-cwlogs==1.4.1 to be made available 
			before we can refer to it in the script."""
            ...
            # For standalone installation we will ignore plugin url and simply install from the dependency folder.
            if self.plugin_url:
                if need_wheel_29:
